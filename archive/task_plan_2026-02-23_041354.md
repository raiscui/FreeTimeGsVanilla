...
# 任务计划: 解读 FreeTimeGS "anytime anywhere" 与 `.splat4d` 的对应关系

- [x] 阶段1: 计划和设置
- [x] 阶段2: 研究/收集信息(摘录论文关键句与公式)
- [x] 阶段3: 对照代码/格式(训练+exporter)
- [x] 阶段4: 输出结论(回复+写 notes/WORKLOG)
- 方向A(最佳): 补一份 specs 文档,把论文公式 -> `.splat4d` timeModel 映射写清,长期防误解.
- 方向B(当前): 只抓最关键原文+对照代码指针,先快速回答当前疑问.
- 结论: "anytime anywhere"=每个 Gaussian 带(µx,µt,s,v)并用时间核控制可见性; per-frame PLY 不需要点数/ID 对齐; `.splat4d` 已承载(time/duration/velocity),其中 v2(gaussian)更贴近论文.
- 状态: [2026-02-23 03:05:17 UTC] 已完成.
